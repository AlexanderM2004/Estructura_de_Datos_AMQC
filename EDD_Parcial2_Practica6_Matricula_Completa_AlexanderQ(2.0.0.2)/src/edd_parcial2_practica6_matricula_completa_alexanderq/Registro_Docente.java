...
public class Registro_Docente extends JFrame{
    ...
    public void ColocarCajatexto(){
        ...
        txt_materia.setBounds(140, 247, 300, 20);
    ...
    public void ColocarComboBox(){
        ...
        Cmb_Carrera.setBounds(140, 207, 300, 20);
    ...
    public void ColocarBotones(){
        ...
        Eliminar.setBounds(310, 300, 100, 30);
        panel.add(Eliminar);
    ...
    private void addRegister(ActionEvent evt) {
        ...
        String carre = (String) Cmb_Carrera.getSelectedItem();
        String mater = (String) txt_materia.getText();
        if (!ci.isEmpty() && !nomb.isEmpty() && !peri.isEmpty() && !esta.isEmpty()&& !carre.isEmpty() && !mater.isEmpty()) {
            //Creamos una nota - todavia no se guarda en la base de datos
            Document doc = new Document("C1", ci).append("Nombre", nomb).append("Periodo", peri).append("Estado", esta).append("Carrera", carre).append("Materia", mater);
            try {
    ...
    private void EliminarActionPerformed(ActionEvent evt) {
        ...
        if (fila != -1) {
            ...
            String carre = (String) Tabla_Docente.getValueAt(fila,4);
            String mater = (String) Tabla_Docente.getValueAt(fila,5);

            // Configurar los criterios de eliminación
            Document criterios = new Document("C1", ci).append("Nombre", nomb).append("Periodo", peri).append("Estado", esta).append("Carrera", carre).append("Materia", mater);
